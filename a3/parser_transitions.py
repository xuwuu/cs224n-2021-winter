...
def minibatch_parse(sentences, model, batch_size):
    ...
    while(idx < len(sentences)):
        partialParses = [PartialParse(
            sentence) for sentence in sentences[idx:min(idx+batch_size,len(sentences)+1)]]
        ...
        while len(unfin_partialParses) != 0:
            ...
            for i, transition in enumerate(transitions):
                unfin_partialParses[i].parse([transition])
            unfin_partialParses = list(
                filter(lambda p: len(p.stack) != 1, unfin_partialParses))
