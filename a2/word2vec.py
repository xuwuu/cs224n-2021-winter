...
def negSamplingLossAndGradient(
    ...
    negz = sigmoid(-np.dot(outsideVectors[negSampleWordIndices],
                  centerWordVec))
    loss = - np.log(z) - np.sum(np.log(negz)) 
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradCenterVec = np.zeros(centerWordVec.shape)

    gradCenterVec = (z-1)*outsideVectors[outsideWordIdx] - \
        np.dot(negz-1, outsideVectors[negSampleWordIndices])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideword in outsideWords:
        loss1, gradCenterVecs1, gradOutsideVectors1 = \
            word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]],
                                    word2Ind[outsideword],
                                    outsideVectors,
                                    dataset)
        loss += loss1
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterVecs1
